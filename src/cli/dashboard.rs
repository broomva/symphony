...
pub async fn launch(dashboard_port: u16, daemon_port: u16) -> anyhow::Result<DashboardHandle> {
    ...
    generate_env_local(&dashboard_dir, daemon_port)?;
    install_deps_if_needed(&dashboard_dir, &runtime).await?;
    run_migrations(&dashboard_dir, &runtime).await?;

    let child = spawn_dashboard(&dashboard_dir, &runtime, dashboard_port).await?;
...
fn generate_env_local(dashboard_dir: &Path, daemon_port: u16) -> anyhow::Result<()> {
...
async fn spawn_dashboard(dashboard_dir: &Path, runtime: &str, port: u16) -> anyhow::Result<Child> {
